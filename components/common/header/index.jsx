		...
		<Container className={classes.container}>
			<Box display="flex" justifyContent="space-between" alignItems="center" mt={2}>
				<Box width="100px" objectFit="cover" mr={2}>
					<Image src={LOGO} alt="logo" size="20px" layout="responsive" width="50px" height="30px" />
				</Box>
				<Box component="nav" className={classes.menu}>
					<Box className={clsx(classes.wrapper_menu_button, classes.menu_background)}>
					...
					<Box className={clsx(classes.wrapper_menu_button, classes.menu_background)} mr={1}>
						<Box className={classes.menu_button}>
							<MoreVertIcon />
						</Box>
					</Box>
			...
			<Box display="flex" className={classes.content_search} mt={1}>
				...
				>
					{data.map((item, index) => (
						<Typography variant="subtitle2" key={index} className={classes.item_autocomplete}>
							{item.name}
						</Typography>
					))}
				</Box>
